...
export function formatBuildingEffect(key: string, val: number) {
  switch (key) {
    ...
    case 'raid_storage_protection_percent': return `+${val}% Proteção do Armazém em incursões`;
    case 'raid_treasury_protection_percent': return `+${val}% Proteção da Tesouraria em incursões`;
    ...
    case 'prison_capacity': return `Capacidade planejada: ${val} cativo${val === 1 ? '' : 's'}`;
    ...
    case 'resonator_shield_percent': return `+${val}% Escudo territorial`;
