...
**Status:** documento canônico atualizado após M5-C.  
**M5-C:** Defense Power, Readiness, guarnição automática, estratégia e snapshot defensivo concluídos.  
**Próxima fase de produto:** S1 — simplificação da progressão do herói; depois M5-D — Mapa Territorial.  
**Scouting:** M6.  
**Raid real:** M7.
...
Antes do Scouting, cada assentamento ganhará uma coordenada fixa `(x,y)` em um mundo/shard. A Sala de Guerra abrirá o **Mapa Territorial**, separado do mapa PvE.
...
➡️ S1       Simplificação da Progressão do Herói
...
## 11. Próxima extensão — Correspondência territorial assíncrona (M6.1)

Após estabilizar Inteligência M6 e antes de liberar raids M7, introduzir uma caixa de correspondência entre Reinos. O objetivo é permitir diplomacia e resposta a eventos mesmo quando os jogadores não estão conectados ao mesmo tempo, sem transformar o jogo em um chat global.

### Escopo inicial

- botão **Correspondência** disponível no Mapa Territorial e no Centro de Comando para todos os estágios territoriais;
- ao selecionar um Reino no mapa, ação **Escrever para este Reino** com destinatário e coordenadas já preenchidos;
- caixa de entrada, enviados, leitura/não lida e notificação persistente para nova carta;
- carta de contraespionagem com origem identificada deve oferecer atalho **Responder** para o Reino atacante;
- limite de tamanho, cooldown por par de Reinos e bloqueio/silenciamento para reduzir spam e assédio;
- mensagens são persistentes e entregues quando o destinatário voltar a ficar online.

### Limites de produto e segurança

- não haverá chat global, presença em tempo real nem exposição de status online;
- cartas não revelam dados privados, coordenadas fora do contrato público ou informações de scouting;
- o remetente só é identificável na contraespionagem quando a regra M6 já o identificou; a correspondência não deve contornar essa proteção;
- denúncias, bloqueio e trilha de auditoria administrativa devem existir antes de abrir a função para todos os jogadores.

### Dependências e ordem

1. consolidar M6: notificação clicável, relatórios e contraespionagem;
2. modelar `kingdom_correspondence` com remetente, destinatário, texto sanitizado, estados de leitura, timestamps e auditoria;
3. implementar inbox/notificações e deep links a partir do mapa e de alertas identificados;
4. validar limites anti-spam e moderação com contas QA;
5. só então seguir para M7, sem tornar a correspondência pré-requisito da raid.
