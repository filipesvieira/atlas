...
func main() {
	...
	defer writer.Flush()
	_ = writer.Write([]string{"item", "slot", "weapon_type", "required_level", "tier", "rarity", "attack", "magic_attack", "defense", "bonus_hp", "bonus_mp", "bonus_str", "bonus_dex", "bonus_int", "crit", "lifesteal", "gold_bonus", "weight", "item_power", "value_gold"})
	...
	for templateIndex, template := range game.ListLootTemplates() {
		...
		for rarityIndex, rarity := range rarities {
			...
			_ = writer.Write([]string{
				...
				strconv.Itoa(item.BonusHP), strconv.Itoa(item.BonusMP), strconv.Itoa(item.BonusSTR), strconv.Itoa(item.BonusDEX), strconv.Itoa(item.BonusINT),
