...
## Complemento UX — 2026-08-28

- `CommunicationConsole` substitui o painel isolado de logs e o chat mundial
  flutuante por abas no mesmo console: **Logs**, **Global**, **Clã** e
  **Reino**. Os dois últimos deixam explícito que ainda dependem de protocolo
  social no servidor; não simulam canais inexistentes.
- A estrutura aceita conversas privadas como abas fecháveis, mas nenhuma aba é
  criada até que exista transporte de mensagem direta autorizado pelo backend.
- HP/MP, habilidades, posturas e suprimentos foram movidos para o interior do
  `GameCanvas`. `SUPR.` continua abrindo a configuração autoritativa de poções
  e o orçamento, sem criar um chat artificial.
